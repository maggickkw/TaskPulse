    ...
    <div className='m-5 p-4'>
        <ModalNewTask isOpen={isModalNewTaskOpen} onClose={() => setIsModalNewTaskOpen(false)} />
            ...
            <div className='mb-4 flex justify-start'>
              ...
              <button className={`px-4 py-2 ${view === "table" ? "bg-gray-300" : "bg-white"}`}
              onClick={() => setView("table")}
              >
                Table
              </button>
            </div>
            {isLoading ? (<div>Loaading task...</div>): view === 'list' ? (
              ...
              view === 'table' && filteredTasks && (
                <div className='w-full'>
                  <DataGrid rows={filteredTasks} columns={columns} getRowId={(row) => row.id} className={dataGridClassNames} sx={dataGridSxStyles(isDarkMode)} />
                </div>
              )
            ) }
    </div>
